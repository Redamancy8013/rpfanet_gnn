...
class AstyxDataset(DatasetTemplate):
    ...
    # def kernel_point_encoding(self, points, bandwidths=[1.5, 2.0]):
    ...
    #         # 计算点云数据的密度估计值
    #         density_values = np.exp(kde.score_samples(points))
    #         points = np.column_stack((points, density_values))

    #     return points[:, [0, 1, 2, 4, 5, 6]]
    ...
    def get_pointcloud(self, idx, pc_type):
        if pc_type == 'lidar':
            ...
            radar_file = self.root_split_path / 'radar_6455' / ('%s.txt' % idx)
            assert radar_file.exists()
            # points = np.loadtxt(str(radar_file), dtype=np.float32, skiprows=2, usecols=(0, 1, 2, 3, 4))
            # return self.kernel_point_encoding(points, bandwidths=[1.5, 2.0])
